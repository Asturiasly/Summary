...
#include <numeric>
    ...
    void SearchServer::AddDocument(int document_id, const std::string& document, DocumentStatus status, const std::vector<int>& ratings) {
        if (document_id < 0)
        {
            throw std::invalid_argument("ID документа не может быть меньше 0.");
        }
    ...
    std::tuple<std::vector<std::string>, DocumentStatus> SearchServer::MatchDocument(const std::string& raw_query, int document_id) const {
        if (!IsValidWord(raw_query))
            throw std::invalid_argument("в тексте запроса содержатся недопустимые символы.");
        else if (!IsValidMinus(raw_query))
            throw std::invalid_argument("нельзя использовать минус без слова или добавлять больше одного минуса перед словом");
        else
        {
    ...
    std::vector<std::string> SearchServer::SplitIntoWordsNoStop(const std::string& text) const {
        ...
        if (!IsValidWord(text))
            throw std::invalid_argument("в тексте документа содержатся недопустимые символы.");
        else
        {
